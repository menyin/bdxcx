...
Page({
  ...
  onShow: function () {
    // 页面显示
    var that = this;
    that.setData({ isHideLoc: getApp().data.isHideLoc });
    //历史关键词搜索数据
    swan.getStorage({
      key: "searchHistory1",
      success: function (res) {
    ...
    if (this.data.searchData) {
      var searchData = this.data.searchData; //搜索字段
      ...
      searchData.jobFun = jobFun;
      searchData.jobFunNames = jobFunNames;
      searchData.calling = calling;
      searchData.callingNames = callingNames;

      getApp().data.isSearch = false;
      if (this.data.searchVal) isFocus = false;
    } else {
      var searchData = {
        page: 1,
        moreSearch: false,
    ...
    this.setData({ searchData: searchData, isFocus: isFocus });
  },
  //获取搜索值
